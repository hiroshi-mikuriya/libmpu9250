...
static int i2c_read_with_init(uint8_t dev_addr, uint8_t reg, uint8_t* buf,
                              int size) {
  int err = 0;
  err = i2c_init(dev_addr);
  if (err) return err;
...
int mpu9250_i2c_init(int dev_addr) {
  ...
  if (err) return err;
  err = i2c_write_with_init(AK8963_ADDR, REG_MAG_CNTL,
                            0x16);  // 磁気センサの出力周期(100Hz)
  if (err) return err;
  s_dev_addr = dev_addr;
  s_mode = I2C_MODE;
  return 0;
...
int mpu9250_read_mag(short* mag) {
  ...
  for (int i = 0; i < 3; ++i)
    mag[i] =
        (short)((buf[i * 2] & 0xFF) + (buf[i * 2 + 1] << 8));  // TODO: use Adj
  return 0;
}
